...
ddr_word_t lane_input_word(const ddr_word_t* input,
                           ...
                           int ddr_word_col) {
#pragma HLS INLINE
    const int lane = lane_base + lane_offset;
...
#pragma HLS INLINE off
    for (int raw_r = 0; raw_r < kIterations; ++raw_r) {
        for (int w = 0; w < kDdrWordsPerRow; ++w) {
            lane0.write(lane_input_word(input, lane_base, 0, raw_r, w));
            lane1.write(lane_input_word(input, lane_base, 1, raw_r, w));
            lane2.write(lane_input_word(input, lane_base, 2, raw_r, w));
            lane3.write(lane_input_word(input, lane_base, 3, raw_r, w));
            lane4.write(lane_input_word(input, lane_base, 4, raw_r, w));
            lane5.write(lane_input_word(input, lane_base, 5, raw_r, w));
            lane6.write(lane_input_word(input, lane_base, 6, raw_r, w));
            lane7.write(lane_input_word(input, lane_base, 7, raw_r, w));
        }
    }
}
...
#pragma HLS INLINE off
    for (int raw_r = 0; raw_r < kIterations; ++raw_r) {
        for (int w = 0; w < kDdrWordsPerRow; ++w) {
            store_lane_output_word(output, lane_base, 0, raw_r, w, lane0.read());
            store_lane_output_word(output, lane_base, 1, raw_r, w, lane1.read());
            store_lane_output_word(output, lane_base, 2, raw_r, w, lane2.read());
            store_lane_output_word(output, lane_base, 3, raw_r, w, lane3.read());
            store_lane_output_word(output, lane_base, 4, raw_r, w, lane4.read());
            store_lane_output_word(output, lane_base, 5, raw_r, w, lane5.read());
            store_lane_output_word(output, lane_base, 6, raw_r, w, lane6.read());
            store_lane_output_word(output, lane_base, 7, raw_r, w, lane7.read());
        }
    }
}
