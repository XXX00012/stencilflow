...
void hdiff_lap(lap_input_buffer& in_window,
               output_buffer<int32_t>& sub_pack) {
  ...
  for (unsigned r = 0; r < hdiff_cfg::kRowsPerCall; ++r) {
    ...
    int32_t* __restrict out_ms_flux1_base = sub_base + out_off + (0 * COL);
    int32_t* __restrict out_ms_flux2_base = sub_base + out_off + (1 * COL);
    int32_t* __restrict out_ms_flux3_base = sub_base + out_off + (2 * COL);
    int32_t* __restrict out_ms_flux4_base = sub_base + out_off + (3 * COL);

    // data() points at the margin/history region followed by the current row.
    int32_t* row0_base = base_ptr + in_off + (0 * COL);
    int32_t* row1_base = base_ptr + in_off + (1 * COL);
    int32_t* row2_base = base_ptr + in_off + (2 * COL);
    int32_t* row3_base = base_ptr + in_off + (3 * COL);
    int32_t* row4_base = base_ptr + in_off + (4 * COL);
